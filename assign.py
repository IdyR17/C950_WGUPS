...
# Before routing, we also have to take the constraints into consideration. The following logic will be implemented on top of Nearest Neighbor:
...
# To be able to deliver in less miles, we use a greedy distance approach to assign the remaining packages


def assign_packages(
    truck1, truck2, truck3, packages, total_packages, addresses, distances, get_distance
):
    # These packages are preloaded because of hard delivery constraints
        ...
        if package.deadline != "EOD":
            closest_truck = find_closest_truck(
                ...
                [
                    truck1,
                    truck2,
                ],  # these trucks leave early, truck 3 stays at HUB until driver returns
                packages,
                addresses,
                distances,
                get_distance,
...
# This is achieved by comparing the new pacakge to pacakges already assigned to each truck
    ...
    for truck in trucks:
        ...
        for assigned_id in truck.package_ids:
            ...
            distance = get_distance(
                package.address, assigned_package.address, addresses, distances
            )
